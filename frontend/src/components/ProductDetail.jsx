  ...
  return (
    <>
      ...
      <div className="max-w-6xl mx-auto px-4 pt-25" dir="rtl">
        <div className="bg-white shadow-xl rounded-2xl border border-blue-200 overflow-hidden flex flex-col lg:flex-row gap-8">
          ...
          <div className="lg:w-1/2 p-6 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm text-blue-900">
              <div>الحالة: <span className="font-medium text-gray-800">{product.condition}</span></div>
              <div>مدة التخزين: <span className="font-medium text-gray-800">{product.storageDuration}</span></div>
              <div>الموديلات الموافقة: <span className="font-medium text-gray-800">{product.compatibleModels}</span></div>
              <div>تقييم البائع: <span className="font-medium text-yellow-500">{product.rating} ★</span></div>
              <div>البائع: <span className="font-medium text-gray-800">{product.seller}</span></div>
            </div>

            <div className="mt-4">
              <h2 className="text-blue-800 font-semibold mb-2">تفاصيل إضافية:</h2>
              <p className="text-gray-700 text-sm leading-relaxed">{product.additionaldetails}</p>
            </div>
        ...
        {isChatOpen && <SellerChat product={product} onClose={() => setIsChatOpen(false)} />}
      </div>
    </>
  );
};
